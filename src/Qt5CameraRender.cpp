...
Qt5CameraRender::Qt5CameraRender(QWidget *parent)
...
Qt5CameraRender::~Qt5CameraRender()
{

    cleanup();
}

void Qt5CameraRender::initializeGL()
{
    if (m_initialized) {
...
void Qt5CameraRender::resizeGL(int w, int h)
{
    glViewport(0, 0, w, h);
}

void Qt5CameraRender::displayImage(const QImage &image)
{
    if (image.isNull()) {
...
void Qt5CameraRender::createTexture(const QImage &image)
{
    // 删除现有纹理
    if (m_textureId != 0) {
...
void Qt5CameraRender::resizeEvent(QResizeEvent *event)
{
    if (isValid() && context()) {
        makeCurrent();
...
bool Qt5CameraRender::initializeShaderProgram()
{
    if (m_shaderProgram) {
...
bool Qt5CameraRender::initializeBuffers()
{
    // 顶点数据：位置(2) + 纹理坐标(2)
...
QOpenGLTexture * Qt5CameraRender::loadTexture(const QImage& image)
{
